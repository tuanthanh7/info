...
class AFthemes_Widget_Base extends WP_Widget
{
     ...
     */
    public function storeship_generate_section_title($field, $label, $note = '', $class = '', $css='')
    {
     ...
     * @param string $note field note to appear below
     */
    public function storeship_generate_section_note($field, $label, $class = '')
    {
        $style = 'text-align:center;background-color:#f1f1f1;padding:5px 0;margin-top:5px;';
        ?>
     ...
     * @param string $note field note to appear below
     */
    public function storeship_generate_text_input($field, $label, $value, $type = 'text', $note = '', $class = '')
    {
     ...
     * @param string $note field note to appear below
     */
    public function storeship_generate_textarea($field, $label, $note = '')
    {
     ...
     */
    public function storeship_generate_checkbox_input($field, $label, $elements, $note = '')
    {
        $instance = isset($this->form_instance[$field]) ? $this->form_instance[$field] : true;
        ?>
        <div class="storeship-multiple-check-form">
            <p>
                <label for="<?php echo esc_attr($this->get_field_id($field)); ?>">
                    <?php echo esc_html($label); ?>
                </label>
     ...
     * @param Array_A $elements
     */
    public function storeship_generate_select_options($field, $label, $elements, $note = '')
    {
    ...
    public function storeship_generate_image_upload($field, $label, $value, $note = '', $class = '')
    {
        $instance = isset($this->form_instance[$field]) ? $this->form_instance[$field] : '';
        ?>
        <div>
            ...
            <div class="image-preview-wrap">
                ...
                <input type="button" class="select-img button button-primary"
                       value="<?php esc_attr_e('Upload', 'storeship'); ?>"
                       data-uploader_title="<?php esc_attr_e('Select Image', 'storeship'); ?>"
                       data-uploader_button_text="<?php esc_attr_e('Select Image', 'storeship'); ?>"/>
                <?php
                $image_status = false;
                if (!empty($instance)) {
                    $image_status = true;
                ...
                ?>
                <input type="button" value="<?php echo esc_attr_x('X', 'Remove', 'storeship'); ?>"
    ...
    {
        $instance = array();
        $instance = $this->storeship_sanitize_data($instance, $new_instance);
        return $instance;
    }

    public function storeship_sanitize_data($instance, $new_instance)
    {
        if (is_array($this->text_fields)) {
            // update the text fields values
            foreach ($this->text_fields as $field) {
                $instance = array_merge($instance, $this->storeship_update_text($field, $new_instance));
            }
        }
        ...
        if (is_array($this->url_fields)) {
            ...
            foreach ($this->url_fields as $field) {
                $instance = array_merge($instance, $this->storeship_update_url($field, $new_instance));
            }
        }
        ...
        if (is_array($this->text_areas)) {
            ...
            foreach ($this->text_areas as $field) {
                $instance = array_merge($instance, $this->storeship_update_textarea($field, $new_instance));
            }
        }
        if (is_array($this->checkboxes)) {
            // update the checkbox fields values
            foreach ($this->checkboxes as $field) {
                $instance = array_merge($instance, $this->storeship_update_checkbox($field, $new_instance));
            }
        }
        if (is_array($this->select_fields)) {
            // update the select fields values
            foreach ($this->select_fields as $field) {
                $instance = array_merge($instance, $this->storeship_update_select($field, $new_instance));
            }
        }
        return $instance;
    }
     ...
     */
    public function storeship_update_text($name, $new_instance)
    {
        $instance = array();
     ...
     * @return object validate new instance
     */
    public function storeship_update_url($name, $new_instance)
    {
        $instance = array();
     ...
     * @return object validate new instance
     */
    public function storeship_update_textarea($name, $new_instance)
    {
        $instance = array();
     ...
     * @return object validate new instance
     */
    public function storeship_update_checkbox($name, $new_instance)
    {
        $instance = array();
     ...
     * @return object validate new instance
     */
    public function storeship_update_select($name, $new_instance)
    {
        $instance = array();
